setwd("Desktop/Rstudio/Sperm")
sperm=read.table('sperm_rfile.csv', header=T)
View(sperm_rfile)
...
#=================================================#
sperm1<-glm(peakcon ~ trtnum + mon + trtnum*mon, data=sperm_rfile)
summary(sperm1)
...
summary(sperm4)
sperm5<-glm(resp ~ trtnum + mon + trtnum*mon, data=sperm_rfile)
summary(sperm5)
...
(sperm_rfile$mon)
monfact<-as.factor(sperm_rfile$mon)
levels(monfact)
sperm_rfile<-cbind(sperm_rfile, monfact=as.factor(sperm_rfile$mon))
head(sperm_rfile)
